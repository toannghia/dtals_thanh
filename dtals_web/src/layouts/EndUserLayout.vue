...
const menuItems: MenuItem[] = [
  { title: 'BẢNG ĐIỀU KHIỂN', icon: '📊', path: '/user' },
  { title: 'XÁC THỰC EKYC', icon: '🛡️', path: '/user/ekyc' },
  { title: 'TÀI KHOẢN NTRIP', icon: '👤', path: '/user/ntrip-accounts' },
  { title: 'ĐƠN HÀNG', icon: '📦', path: '/user/orders' },
  { title: 'HỖ TRỢ KỸ THUẬT', icon: '💬', path: '/user/support' },
];
</script>
...
<template>
  <div v-if="isLoaded" class="flex h-screen w-full overflow-hidden bg-[#f4f6f9]">
    <!-- Sidebar -->
    <aside :class="['h-full bg-[#343a40] transition-all duration-300 shadow-xl z-[1001] flex-shrink-0', isCollapse ? 'w-[64px]' : 'w-[250px]']">
      <div class="sidebar-container h-full flex flex-col bg-[#343a40]">
        ...
        <div class="brand-link flex items-center h-[70px] px-5 bg-[#3d444b] border-b border-gray-700">
          ...
          <span v-if="!isCollapse" class="text-white font-bold text-lg tracking-wider">
            DTALS <small class="font-light opacity-50">Portal</small>
          </span>
        </div>
    ...
    <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <div class="flex items-center gap-4 pr-6">
          <el-dropdown trigger="click">
            <div class="flex items-center gap-2 cursor-pointer text-gray-600 hover:text-blue-500 transition-colors">
              ...
              <span class="text-sm">{{ authStore.user?.username || 'User' }}</span>
            </div>
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item @click="$router.push('/user/profile')">👤 Hồ sơ cá nhân</el-dropdown-item>
      ...
      <main class="flex-1 overflow-y-auto p-6 scroll-smooth bg-[#f4f6f9]">
        ...
        <footer class="mt-8 py-4 border-t border-gray-200 text-center text-xs text-gray-400">
          <strong>DTALS User Portal</strong> &copy; 2026. Phát triển bởi <b>Zenpos Team</b>.
        </footer>
      </main>
    </div>
  </div>
